...
The Edge Agent is the workhorse of Factory+, operating at the periphery, gathering data from devices, normalising it into Schemas, converting the packets into Sparkplug messages, and transmitting them to the MQTT component.
...
- **MUST** respond to `Node Control/Reload Edge Agent Config` NCMDs messages, which **MUST** instruct the Edge Agent to fetch a fresh configuration from the Manager
