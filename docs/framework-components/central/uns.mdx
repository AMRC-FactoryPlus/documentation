...
The UNS is one of two namespaces Factory+ components publish and subscribe to. The Sparkplug namespace facilitates data
...
while the UNS serves as a means of communication for components at levels 3 and 4 (MES and ERP systems). Data from
...
* All topics **MUST** be prefixed with the ISA-95 structure.
...
* Topics **MUST**  state their source (Edge, MES, etc.)
...
batched metrics where the "Batch" property is an array of values and timestamps.

| Property    | Type   | Description                                         |
|-------------|--------|-----------------------------------------------------|
| `Value`     | string | The value to publish                                |
| `Timestamp` | string | The timestamp of the value at source                |
| `Batch`     | array  | Array of value and timestamps to publish            |
...
| `Unit`             | string  | The measurment unit of the value               |
