...
ConfigDB and observes its output. If a Node does not publish any packets
...
Edge Agent fetches its configuration from the ConfigDB at startup; along
with the configuration, the ConfigDB returns an HTTP Etag identifying
this particular revision of the config entry. The Edge Agent publishes
this information in its Node birth certificate.

The edge monitor watches for change-notify messages from the ConfigDB
