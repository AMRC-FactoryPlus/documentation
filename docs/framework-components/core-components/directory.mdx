...
As per the [Component Specification](/docs/framework-components/specification), the Directory component provides a MQTT/Sparkplug interface and a HTTP interface.

<WellKnownUUID type='Service Function' name='Directory'
    uuid='af4a1d66-e6f7-43c4-8a67-0fa3be2b1cf9'
    description='Directory service specification.'/>
...
| `Device_UUID`    | UUID   | Published when a device has come online or offline                                                                                                                                                                                                           |
| `Device_Address` | string | Published when a device has come online or offline                                                                                                                                                                                                           |
...
The Directory service itself only defines the UUID for the Directory
service function. However the other Service Function UUIDs defined by
the Factory+ spec are listed below for convenience.
...
If a Sparkplug device includes this schema in its Birth Certificate, the
Directory will recognise it as a service advertisment and publish the
supplied URL over HTTP.
...
This group must be created in the Auth service containing the permission
UUIDs listed below.
...
This grants permission for a given account to advertise the given
service.
...
This grants the 'principal' account (normally an administrator account,
or an operator account performing this specific function) permission to
advertise services for the 'target', which also represents an account.
This allows use of the `/v1/service/{service}/advertisment/{owner}`
endpoint.

In addition to the 'principal' account having this permission on the
'target account, either

* the 'target' account must have 'Advertise Service', or

* the 'principal' account must have 'Override Service Advertisment
Permissions'
...
This grants an account permission to advertise services on behalf of
another account that the other account cannot advertise on its own
behalf. The 'target' of the permission is the service function UUID
being advertised.
