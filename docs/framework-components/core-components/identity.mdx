...
Factory+ utilises Kerberos as its identity provider to enhance the security and authentication processes within the system. By utilising Kerberos, Factory+ ensures that users and services can authenticate themselves in a secure and reliable manner, while also allowing for seamless integration with external systems that rely on the same authentication mechanism and/or other Factory+ deployments.
