...
central cluster via the ConfigDB. The operator has two roles: updating
the status of the cluster in the ConfigDB, and arranging to deploy edge
...
taint is made available in the ConfigDB and can be used to provide
...
pushes the certificate up to the ConfigDB so that the Cluster Manager
