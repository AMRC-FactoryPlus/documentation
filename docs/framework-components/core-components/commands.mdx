...
All permissions for the Commands component **MUST** be members of the above `Commands` permission group. Each permission **MUST** have an entry in the Configuration Store under
the `Commands` application. This entry defines the metrics which are permitted, and consists of an array of objects with `name` and `type` fields. The name is required, and gives the metric name. The type is optional, and should be specified as one of the type names defined in the Sparkplug spec.
