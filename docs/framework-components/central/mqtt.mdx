...
Similar to the [Data Warehouse Component](/docs/framework-components/core-components/data-warehouse), Factory+ does not prescribe a particular specification for the MQTT Component; it only necessitates that any chosen solution be MQTT 3.1 compatible. There are however a few requirements of any chosen MQTT broker that must be satisfied to ensure compatibility with Factory+.

### Authentication & Authorisation
Any chosen MQTT broker must be able to authenticate users via Kerberos and enforce fine-grained access control for topic management via rules from the [Authorisation Component](/docs/framework-components/core-components/authorisation).
...
These well-known UUIDs are part of the core framework and all **MUST** to be registered with the [Configuration Store](/docs/framework-components/core-components/configuration-store) component under the appropriate classes.
